...
import { Lock, Save, UserX } from "lucide-react";
...
export default function MatchesPage() {
  ...
  return (
    ...
    >
      <div>
        <h1 className="text-3xl font-black tracking-tight text-zinc-900 dark:text-white">Matches</h1>
        <p className="mt-1 text-sm text-zinc-500">Predict the scorelines before kickoff.</p>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center justify-between border-b border-zinc-100 bg-zinc-50/50 px-5 py-3 backdrop-blur-md dark:border-zinc-800/50 dark:bg-zinc-900/50">
                <span className="text-xs font-bold uppercase tracking-wider text-zinc-500">Group {homeTeam.group}</span>
              ...
              <div className="flex items-center justify-between p-6 sm:p-8">
                ...
                <div className="flex w-1/3 flex-col items-center gap-3">
                  <div className="flex h-14 w-14 overflow-hidden items-center justify-center rounded-2xl bg-zinc-100 shadow-inner dark:bg-zinc-800">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={`https://flagcdn.com/w80/${homeTeam.iso2}.png`} alt={homeTeam.name} className="h-full w-full object-cover" />
                  </div>
                  <span className="text-center text-sm font-black tracking-tight text-zinc-900 dark:text-white sm:text-lg">{homeTeam.name}</span>
                </div>
                ...
                <div className="flex w-1/3 flex-col items-center gap-3">
                  <div className="flex h-14 w-14 overflow-hidden items-center justify-center rounded-2xl bg-zinc-100 shadow-inner dark:bg-zinc-800">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={`https://flagcdn.com/w80/${awayTeam.iso2}.png`} alt={awayTeam.name} className="h-full w-full object-cover" />
                  </div>
                  <span className="text-center text-sm font-black tracking-tight text-zinc-900 dark:text-white sm:text-lg">{awayTeam.name}</span>
                </div>
              </div>
